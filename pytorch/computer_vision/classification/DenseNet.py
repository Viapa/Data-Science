...
@target: 学会加载预训练DenseNet121模型, 并在训练中加入学习率控制函数, 打印每个批次当前的学习率
"""
...
# 定义训练函数
def train(epochs, train_loader, valid_loader, model, loss_fn, optimizer, scheduler):
    best_score = 0.1
...
if __name__ == "__main__":
    ...
    model.classifier = nn.Sequential(
        nn.Linear(model.classifier.in_features, len(idx_to_classes))
    )
